...
@pytest.mark.parametrize("dtype_str,M_BLOCK,N_BLOCK", [
    ("bfloat16", 32, 128),
    ("float32", 32, 64),
])
def test_tensor_descriptor_load_partial_reduce(dtype_str, M_BLOCK, N_BLOCK, device):
    ...
    out = torch.empty((M // M_BLOCK, N), dtype=torch.float32, device=device)

    kernel[grid](out, desc, M, N, M_BLOCK, N_BLOCK, num_warps=4)
